...
class ResNetBottom(nn.Module):
    def __init__(self, original_model, n_attributes, backbone, num_classes):
        ...
        if n_attributes > 0:
            for i in range(n_attributes):
                if backbone == 'resnet101':
                    self.all_fc.append(FC(2048, 1, expand_dim=0))
                elif backbone == 'resnet18_cub':
                    self.all_fc.append(FC(8192, 1, expand_dim=0))
                elif backbone == 'resnet34':
                    self.all_fc.append(FC(512, 1, expand_dim=0))
        else:
            if backbone == 'resnet101':
                self.all_fc.append(FC(2048, num_classes, expand_dim=0))
            elif backbone == 'resnet18_cub':
                self.all_fc.append(FC(8192, num_classes, expand_dim=0))
            elif backbone == 'resnet34':
                self.all_fc.append(FC(512, num_classes, expand_dim=0))

        for m in self.all_fc.modules():
...
class ViTBottom(nn.Module):
    def __init__(self, original_model, n_attributes, backbone, num_classes):
        ...
        self.process_input = original_model._process_input
        self.class_token = original_model.class_token
        self.encoder = original_model.encoder
        self.heads = original_model.heads
        self.all_fc = nn.ModuleList()

        if n_attributes > 0:
            for i in range(n_attributes):
                if backbone == 'vit_b16':
                    self.all_fc.append(FC(768, 1, expand_dim=0))
                elif backbone == 'vit_l16':
                    self.all_fc.append(FC(1024, 1, expand_dim=0))
        else:
            if backbone == 'vit_b16':
                self.all_fc.append(FC(768, num_classes, expand_dim=0))
            elif backbone == 'vit_l16':
                self.all_fc.append(FC(1024, num_classes, expand_dim=0))
...
def get_backbone(backbone, n_attributes, num_classes):
    from torchvision.models import resnet101, ResNet101_Weights, resnet34, ResNet34_Weights, vit_b_16, ViT_B_16_Weights, vit_l_16, ViT_L_16_Weights
    from pytorchcv.model_provider import get_model as ptcv_get_model

    if backbone == 'resnet101':
        model=resnet101(weights=ResNet101_Weights.IMAGENET1K_V2)
        backbone = ResNetBottom(original_model=model, n_attributes=n_attributes, backbone=backbone, num_classes=num_classes)
    ...
    if backbone == 'resnet34':
        ...
        backbone = ViTBottom(original_model=model, n_attributes=n_attributes, backbone=backbone, num_classes=num_classes)
...
class End2EndModel(torch.nn.Module):
    def __init__(self, model1, model2, use_relu=False, use_sigmoid=False, n_class_attr=2, backbone=''):
