...
def calculate_etp_score(criterion, b_class_logits, intervened_idx, ptl_5, ptl_95, model2, b_attr_outputs, b_attr_outputs_sigmoid, b_class_labels, 
    ...
    n_attributes = int(len(b_attr_outputs_sigmoid) / len(b_class_labels))
...
def calculate_c_score(criterion, attr_preds_sigmoid, attr_labels, model2, seed=1, model_dir='', class_labels=0):
    if criterion == 'lcp':
...
def calculate_intervention_order(criterion, replace_val, preds_by_attr, ptl_5, ptl_95, model2, b_attr_binary_outputs, b_class_labels, b_class_logits,
                                ...
                                n_replace, use_relu, use_sigmoid, batch_intervention, n_trials=1, connect_CY=False, class_label='binary', seed=1, model_dir=''):
        ...
        if criterion in ['ectp', 'eudtp']:
            ...
            if batch_intervention:
                if criterion == 'rand':
                    ...
                    score_list = calculate_c_score(criterion, b_attr_outputs_sigmoid, b_attr_labels, model2, seed, model_dir)
                ...
                for img_id in range(len(b_class_labels)):
                    ...
                    elif criterion == 'rand_wrong':
                        intervention_order = [i for i in range(args.n_attributes) if round(attr_preds[i]) == attr_labels[i]]
                    else:
                        score = calculate_c_score(criterion, attr_preds_sigmoid, attr_labels, model2, seed=seed, model_dir=model_dir, class_labels=b_class_labels[img_id])
...
def simulate_intervention(intervention_order, criterion, replace_val, preds_by_attr, ptl_5, ptl_95, model2, b_attr_binary_outputs, b_class_labels, b_class_logits,
                                b_attr_outputs, b_attr_outputs_sigmoid, b_attr_outputs2, b_attr_labels,
                                b_attr_intervened,
                                n_replace, use_relu, use_sigmoid, batch_intervention, trial, 
    ...
    if n_replace > 0:
        if batch_intervention:
            ...
            for img_id in range(len(b_class_labels)):
                if criterion == 'rand_wrong':
                    if n_replace - 1 < len(intervention_order[trial][img_id]):
        ...
        if criterion == 'rand_wrong':
            labels = np.array(b_attr_labels)[attr_replace_idx]
            b_attr_new[attr_replace_idx] = 1 - labels
        else:
...
def run(args):
    ...
    _, _, _, b_class_labels, b_topk_class_outputs, b_class_logits, b_attr_labels, b_attr_outputs, b_attr_outputs_sigmoid, \
    ...
    assert args.criterion in ['rand', 'ucp', 'lcp', 'cctp', 'ectp', 'eudtp', 'rand_wrong']
