...
from CUB.config import N_ATTRIBUTES, N_CLASSES, DATA_DIR
...
def get_class_attributes_data(min_class_count, out_dir, data_dir, modify_data_dir='', keep_instance_data=False):
    """
    ...
    create_new_dataset(out_dir, 'attribute_label', collapse_fn, data_dir=data_dir)#=modify_data_dir)
...
def create_logits_data(model_path, out_dir, data_dir='', use_relu=False, use_sigmoid=False, backbone='inception_v3'):
    ...
    get_logits_train = lambda d: inference(d['img_path'], model, use_relu, use_sigmoid, is_train=True, backbone=backbone)
    get_logits_test = lambda d: inference(d['img_path'], model, use_relu, use_sigmoid, is_train=False, backbone=backbone)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_value', default=10, type=int, help='Number of samples for data efficiency split')
    ...
    elif args.exp == 'MajorityVoting':
        get_class_attributes_data(args.min_value, args.out_dir, args.data_dir, keep_instance_data=False)
    elif args.exp == 'Reduced':
        get_class_attributes_data(10, args.out_dir, args.data_dir, keep_instance_data=True)
