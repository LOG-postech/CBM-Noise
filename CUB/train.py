...
def run_epoch_simple(model, optimizer, loader, loss_meter, acc_meter, criterion, args, is_training):
    ...
    for _, data in enumerate(loader):
        ...
        if is_training:
            if args.optimizer in ['SAM', 'SAM_ADAMW']:
                loss.backward()
...
def train(model, args):
    ...
    if args.dataset == 'awa2':
        ...
        VAL_CSV = os.path.join(args.val_dir, "val.csv")
        TEST_CSV = ""
        train_loader,val_loader,_,imbalance,_= generate_data(args, args.predicate_dir, TRAIN_CSV, VAL_CSV, TEST_CSV, resol=299)
        ...
        train_data_path = os.path.join(BASE_DIR, args.data_dir, 'train.pkl')
        val_data_path= os.path.join(BASE_DIR, args.val_dir, 'val.pkl')
        ...
        if args.ckpt: #retraining
            train_loader = load_data([train_data_path, val_data_path], args.use_attr, args.no_img, args.batch_size, args.uncertain_labels, image_dir=args.image_dir, \
                                    n_class_attr=args.n_class_attr, resampling=args.resampling, args=args)
            val_loader = None
        else:
    ...
    if args.optimizer == 'Adam':
        ...
        optimizer = SAM(model.parameters(), base_optimizer, lr=args.lr, momentum=0.9, weight_decay=args.weight_decay, rho=args.rho)
    elif args.optimizer == 'SAM_ADAMW':
        base_optimizer = torch.optim.AdamW
        optimizer = SAM(model.parameters(), base_optimizer, lr=args.lr, weight_decay=args.weight_decay, rho=args.rho)
    else:
...
def train_X_to_C(args):
    model = ModelXtoC(pretrained=args.pretrained, num_classes=args.n_classes, use_aux=args.use_aux,
                      n_attributes=args.n_attributes, expand_dim=args.expand_dim, three_class=args.three_class, backbone=args.backbone)
    train(model, args)
...
def train_X_to_C_to_y(args):
    model = ModelXtoCtoY(n_class_attr=args.n_class_attr, pretrained=args.pretrained,
                         num_classes=args.n_classes, use_aux=args.use_aux, n_attributes=args.n_attributes,
                         expand_dim=args.expand_dim, use_relu=args.use_relu, use_sigmoid=args.use_sigmoid, backbone=args.backbone)
    train(model, args)

def train_X_to_y(args):
    model = ModelXtoY(pretrained=args.pretrained, num_classes=args.n_classes, use_aux=args.use_aux, backbone=args.backbone)
    train(model, args)

def train_X_to_Cy(args):
    model = ModelXtoCY(pretrained=args.pretrained, num_classes=args.n_classes, use_aux=args.use_aux,
...
def parse_arguments(experiment):
    ...
    if experiment == 'Probe':
        ...
        parser.add_argument('-val_dir', default='val_official_datasets', help='directory to the validation data')
        ...
        parser.add_argument('-rho', type=float, default=0.05, help='hyperparameter for SAM')
        parser.add_argument('-n_classes', type=int, default=N_CLASSES, help='number of classes')
        parser.add_argument('-noise', type=float, default=0.10, help='noise rate')
